...
FenFields parse_fen(const std::string_view fen_string) {
    if (std::count(fen_string.begin(), fen_string.end(), ' ') != 5) {
        throw FenError("FEN must have six fields");
    }
...
Position::Position(const std::string_view fen_string) {
    ...
    if (fen_string == "startpos") { return; }

    FenFields fen_info = parse_fen(fen_string);
    ...
    clear_board();
    {
        ...
        for (std::size_t token_index = 0; token_index < fen_info.board.length(); token_index++) {
            ...
            if (std::string("pnbrqkPNBRQK12345678/").find(token) == std::string::npos) {
                throw FenError("Invalid board piece");
            }

            if (token == '/') {
                if (rank_square_count != 8) {
                    throw FenError("Rank must have 8 squares");
                } else {
                    rank_square_count = 0;
                    continue;
                }
                ...
                if (std::isdigit(fen_info.board[token_index + 1])) {
                    throw FenError("Consecutive digits in board");
                }
                ...
                if ((token == 'P' || token == 'p') && (square.rank() == 0 || square.rank() == 7)) {
                    throw FenError("Pawn on invalid rank");
                }
        ...
        if (white_kings != 1 || black_kings != 1) {
            throw FenError("Invalid king count");
        }
        if (rank_square_count != 8) {
            throw FenError("Rank must have 8 squares");
        }
        if (fen_index != 64) {
            throw FenError("Board must have 8 ranks");
        }
    }

    if (fen_info.turn != "w" && fen_info.turn != "b") {
        throw FenError("Invalid side to move");
    }
    ...
    if (fen_info.castling_rights != "-") {
        ...
        for (char c : fen_info.castling_rights) {
            ...
            if (pos == std::string::npos) {
                throw FenError("Invalid castling rights");
            } else {
                ...
                switch(candidate_option.color()) {
                    case Color::White:
                        if (piece_at(Square('e', '1')) != Piece(Color::White, PieceType::King)) {
                            throw FenError("Missing white king for castling");
                        } break;
                    case Color::Black:
                        if (piece_at(Square('e', '8')) != Piece(Color::Black, PieceType::King)) {
                            throw FenError("Missing black king for castling");
                        } break;
                }
                ...
                switch (candidate_option.encoding()) {
                    case CastlingType::WhiteKingside:
                        if (piece_at(Square('h', '1')) != Piece(Color::White, PieceType::Rook)) {
                            throw FenError("Missing white rook for castling");
                        } break;
                    ...
                    case CastlingType::BlackKingside:
                        if (piece_at(Square('h', '8')) != Piece(Color::Black, PieceType::Rook)) {
                            throw FenError("Missing black rook for castling");
                        } break;
    ...
    if (fen_info.en_passant_target == "-") {
        ...
        if (fen_info.en_passant_target.length() != 2) {
            throw FenError("Invalid en passant square");
        }

        if (fen_info.en_passant_target[0] < 'a' || fen_info.en_passant_target[0] > 'h') {
            throw FenError("Invalid en passant file");
        }
        if (fen_info.en_passant_target[1] != '3' && turn_ == Color::Black) {
            throw FenError("Invalid en passant rank");
        }
    ...
    if (fen_info.halfmove_clock.length() > 6 || fen_info.halfmove_clock.empty()) {
        throw FenError("Invalid halfmove clock");
    }
    for (unsigned char c : fen_info.halfmove_clock) {
        if (!std::isdigit(c)) { throw FenError("Invalid halfmove clock"); }
    }
    halfmove_clock_ = std::stoi(fen_info.halfmove_clock);

    if (fen_info.fullmove_number.length() > 6 || fen_info.fullmove_number.empty()) {
        throw FenError("Invalid move clock");
    }
    for (unsigned char c : fen_info.fullmove_number) {
        if (!std::isdigit(c)) { throw FenError("Invalid move clock"); }
    }
    ...
    if (is_attacked_square(*this, king_square(opposite_turn()), turn_)) {
        throw FenError("Side not to move is in check");
    }
}
